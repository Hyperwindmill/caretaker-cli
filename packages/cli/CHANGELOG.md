...
- 4d9b420: Give agents a working email capability — **send and read** — on top of the `email` service record.
  ...
  `allowedAgents` decides which agents may use an account at all (a scoped account is _invisible_ to
...
- eb7af25: Rename the **Scheduler** settings surface to **Services** and add an **Email (IMAP)**
...
- 6c64f44: chore: add `docker-compose.mail.yml`, a local IMAP server for exercising the **Email (IMAP)**
...
- ce0fa39: Drop the last GPL dependency: the TUI wordmark is now a static banner (generated
...
- 3133b75: fix(desktop): white screen on launch (0.14.1)
...
- d078b90: fix(webview): settings edit forms no longer overflow their frame on short viewports
...
- ebe9181: fix(tasks): bootstrap commands now resolve version-manager binaries (pnpm, npx, …)
...
- aa6fc4e: Publish the CLI to npm as `@hyperwindmill/caretaker-cli`.
