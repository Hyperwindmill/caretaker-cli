...
import { expandTemplate, parseSlashInvocation, tokenizeArgs } from './loader.js';
...
describe('listActiveCommands / resolveCommand', () => {
  ...
  it('returns null when the command exists only on a non-active plugin', async () => {
    await seed([pluginRecord({ name: 'secret', commands: { hidden: { body: 'x' } } })]);
