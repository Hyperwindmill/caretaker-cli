...
test("loadContextFiles: walks up, finds AGENTS.md", async () => {
  ...
  assert.ok(paths.some((p) => p.endsWith("AGENTS.md")), `expected AGENTS.md, got ${paths.join(", ")}`);
...
test("resolveFileReferences: single pass, no recursive expansion", async () => {
  ...
  assert.match(out, /outer with @\.\/inner\.md ref/, "the @./inner.md inside outer.md must NOT be expanded");
  await rm(root, { recursive: true, force: true });
});
