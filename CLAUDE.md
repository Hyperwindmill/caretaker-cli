...
`packages/cli/src/harness/loop.ts` is the chat loop. For each turn it calls `packages/cli/src/harness/provider.ts` (an OpenAI-compatible client) and dispatches tool calls. The set of tools available to a turn is **not** the agent's stored config — it is the output of `packages/cli/src/harness/tools/resolve.ts` (`resolveAgentTools`), which intersects what the agent opted into with what the registry exposes, layers on auto-injected builtins, and applies the tri-state policy (`[ ]` off, `[x]` allowed, `[!]` confirm-each-call). The auto-injection has three tiers, kept narrow on purpose:
...
- **Memory-gated**: `mcp__memory__memory_read` is added when `MemoryConfig` is present in `caretaker.json` — the same gate that injects the `<memories>` prelude block (layer 3). The tool is the other half of that harness channel, not an agent capability: it is marked `Tool.hidden` and excluded from all three tool pickers, so there is no picker off-state for the runtime to contradict — the user-visible switch is `MemoryConfig` itself, and reads are never permission-prompted (the recall bump is the subsystem's own bookkeeping, not an external side effect).
...
6. `<memories>` block (top-K titles from host-side lexical match on user message, gating on `MemoryConfig` present; skipped for memory sweep summarize runs; in claude-code joins `--append-system-prompt`).
...
- **Memory sweep** (`scheduler/memory_sweep.ts`): steps 1 & 2 of the memory
  subsystem (specs: `docs/superpowers/specs/2026-08-24-memory-daemon-step1-design.md`,
  `docs/superpowers/specs/2026-08-24-memory-daemon-step2-extraction-design.md`).
  Gated on `memory` in `caretaker.json` (`MemoryConfig`: `agentId` — the
  *agent* whose identity runs the summarize+extract calls — plus `sweepMinutes`
  ...
  `sweepMinutes` (in-process interval + overlap gates), the sweep walks
  `sessions/<agentId>/*.jsonl` from disk — sessions written by any surface are
  picked up — and maintains one `SessionDigest` per session in the folder DB: a
  cursor (`lastMessageId`/`messageCount`) plus a rolling summary produced by a
  ...
  chunks under a char budget with the cursor persisted per chunk (crash loses
  at most one chunk). The per-chunk call is combined: `{"summary": "...", "memories": [...]}`
  JSON — same model call count; unparsable JSON fails the chunk (cursor stays,
  next sweep retries; deliberately no raw-text fallback, it would poison the digest).
  Extracted memories are persisted to the durable `memories` collection (NOT a
  regenerable cache, unlike the digests) with host-side scope: the model picks
  only `level: 'project' | 'global'`; `projectId` is resolved host-side by
  prefix-matching the session agent's `workingDir` against `projects[].workingDir`
  (no match → global-only). Classification is `kind: 'fact' | 'episode'` +
  `importance: 'low' | 'normal' | 'high'` derived from conversation tone. Dedup
  via existing titles+keywords in the prompt; append-only (no supersede — that
  is future consolidation); memories are saved before the digest record on purpose.
  At most `MAX_CALLS_PER_SWEEP` (10) model calls per sweep, the rest waits.
  `scannedAt` is written **only when a digest is fully caught up**, which is
  what makes the cheap mtime skip (`fileMtime < scannedAt`) sound — a budget/failure
  stop keeps the gate open. The digests collection is a regenerable cache:
  deleting it costs one full re-scan, while memories survive it. Failures are
  best-effort per session, never blocking. Configured from the **Memory** settings
  tab (webview-ui `MemoryTab.tsx` — agent picker + the two numeric fields;
  saves via the ordinary `saveConfig` message, no dedicated endpoint), gated
  `layout === 'sidebar'` like Services/Voice since the sweep is web-server-only.
  Step 3 read path (`docs/superpowers/specs/2026-08-25-memory-daemon-step3-recall-design.md`):
  per-turn host-side lexical match injects `<memories>` block (top-K titles)
  into the prelude; `mcp__memory__memory_read` returns bodies on demand and
  bumps recall accounting. Recall works on every surface (folder DB, no scheduler
  needed), unlike the sweep. Autonomous task cycles (developer/planner/review)
  pass the project scope explicitly (`RunOptions.memoryProjectId`, from the task
  record — still host-side): their workingDir is a worktree or managed repo dir
  that never prefix-matches the project's `workingDir`, so dir-based resolution
  would degrade them to global-only.
