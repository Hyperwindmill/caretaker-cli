...
Because the container runs as **your host user** (not root — so files it writes stay yours), a global install like `npm install -g pnpm` fails with `EACCES` (the global dir is root-owned). Put your **package manager in the image** (a custom image, e.g. `FROM node:24` + `RUN corepack enable pnpm`) and use `bootstrapCommands` only for **dependencies** (`pnpm install`). Installing a package manager at runtime as a non-root user (`npm i -g …`) won't work, and one-shot workarounds like `npx pnpm install` don't leave `pnpm` on `PATH` for later cycles.
